...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!currentImage) {
      setError(t('errors.noImageLoaded'));
      return;
    }
    ...
    try {
        ...
        setError(`${t('errors.failedToGenerate')} ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleApplyFilter = useCallback(async (filterPrompt: string) => {
    if (!currentImage) {
      setError(t('errors.noImageToFilter'));
      return;
    }
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`${t('errors.failedToFilter')} ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleApplyAdjustment = useCallback(async (adjustmentPrompt: string) => {
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`${t('errors.failedToAdjust')} ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleApplyCrop = useCallback(() => {
    if (!completedCrop || !imgRef.current) {
        setError(t('errors.noCropSelection'));
        return;
    }
    ...
    if (!ctx) {
        setError(t('errors.cropProcessError'));
        return;
    }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        <div className="relative w-full shadow-2xl rounded-xl overflow-hidden bg-black/20">
            {isLoading && (
                <div className="absolute inset-0 bg-black/70 z-30 flex flex-col items-center justify-center gap-4 animate-fade-in">
                    <Spinner />
                    <p className="text-gray-300">{t('editor.loadingMessage')}</p>
                </div>
            )}
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'crop', 'adjust', 'filters'] as const).map(tab => (
                 <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`w-full capitalize font-semibold py-3 px-5 rounded-md transition-all duration-200 text-base ${
                        activeTab === tab 
                        ? 'bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-lg shadow-purple-500/40' 
            ...
            {activeTab === 'retouch' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-gray-400">
                        {editHotspot ? t('editor.retouch.promptHelper') : t('editor.retouch.promptHelperInitial')}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            ...
                            placeholder={editHotspot ? t('editor.retouch.placeholder') : t('editor.retouch.placeholderInitial')}
                            className="flex-grow bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-5 text-lg focus:ring-2 focus:ring-blue-500 focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60"
                            disabled={isLoading || !editHotspot}
                        />
                        <button 
                            type="submit"
                            className="bg-gradient-to-br from-blue-600 to-purple-500 text-white font-bold py-5 px-8 text-lg rounded-lg transition-all duration-300 ease-in-out shadow-lg shadow-purple-500/20 hover:shadow-xl hover:shadow-purple-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner disabled:from-blue-800 disabled:to-purple-700 disabled:shadow-none disabled:cursor-not-allowed disabled:transform-none"
                            disabled={isLoading || !prompt.trim() || !editHotspot}
                        >
                            {t('editor.retouch.generateButton')}
                        </button>
                    </form>
                </div>
            )}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            ...
            >
                <UndoIcon className="w-5 h-5 me-2" />
            ...
            >
                <RedoIcon className="w-5 h-5 me-2" />
            ...
            >
                {t('editor.toolbar.uploadNew')}
            </button>

            <button 
                onClick={handleDownload}
                className="flex-grow sm:flex-grow-0 ms-auto bg-gradient-to-br from-blue-600 to-purple-600 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-purple-500/20 hover:shadow-xl hover:shadow-purple-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base"
            >
